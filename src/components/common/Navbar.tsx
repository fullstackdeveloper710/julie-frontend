...
export function Navbar() {
    ...
    // ---------- MARKETING / PUBLIC HEADER (unchanged behavior) ----------
    return (
            ...
            <div className="mx-auto md:px-7 px-3">
                <div className="flex items-center justify-between h-20">
                    ...
                    <div className="flex items-center gap-4 sm:gap-8">
                        ...
                        <div className="hidden navBar sm:flex items-center gap-4">
                            <Link href="/" className={`text-sm font-semibold transition-colors ${isActive('/') ? 'active' : 'text-slate-300 hover:text-white'}`}>HOME</Link>
                            <Link href="/about" className={`text-sm font-semibold transition-colors ${isActive('/about') ? 'active' : 'text-slate-300 hover:text-white'}`}>ABOUT</Link>
                            <Link href="/platform-demo" className={`text-sm font-semibold transition-colors ${isActive('/platform-demo') ? 'active' : 'text-slate-300 hover:text-white'}`}>PLATFORM DEMO</Link>
                            <Link href="/pricing" className={`text-sm font-semibold transition-colors ${isActive('/pricing') ? 'active' : 'text-slate-300 hover:text-white'}`}>PRICING</Link>
                            ...
                            {accessToken ? (
                                <div className="relative">
                                    ...
                                    >
                                        ...
                                        <span className="text-xs text-slate-300 max-w-30 truncate hidden sm:inline">{userEmail}</span>
                                    </Button>
                                    {showUserMenu && (
                ...
                >
                    <div className="space-y-3 px-2 pt-2 pb-4">
                        <Link href="/" onClick={() => setShowMobileMenu(false)} className={`block text-sm font-semibold rounded-md px-3 py-2 transition-colors ${isActive('/') ? 'bg-slate-800  active' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>HOME</Link>
                        <Link href="/about" onClick={() => setShowMobileMenu(false)} className={`block text-sm font-semibold rounded-md px-3 py-2 transition-colors ${isActive('/about') ? 'bg-slate-800  active' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>ABOUT</Link>
                        <Link href="/platform-demo" onClick={() => setShowMobileMenu(false)} className={`block text-sm font-semibold rounded-md px-3 py-2 transition-colors ${isActive('/platform-demo') ? 'bg-slate-800  active' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>PLATFORM DEMO</Link>
                        <Link href="/pricing" onClick={() => setShowMobileMenu(false)} className={`block text-sm font-semibold rounded-md px-3 py-2 transition-colors ${isActive('/pricing') ? 'bg-slate-800  active' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>PRICING</Link>

                        {accessToken ? (
                            <>
                                <Link href="/dashboard" onClick={() => setShowMobileMenu(false)} className="block text-sm font-semibold rounded-md px-3 py-2 text-slate-300 hover:bg-slate-800 hover:text-white transition-colors uppercase">Dashboard</Link>
                                <button
                                    onClick={() => {
                                        setShowMobileMenu(false);
                                        handleLogout();
                            ...
                            <>
                                <Link href="/auth/signin" onClick={() => setShowMobileMenu(false)} className={`block text-sm font-semibold rounded-md px-3 py-2 transition-colors ${isActive('/auth/signin') ? 'bg-slate-800  active' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>LOGIN</Link>
                                <Link href="/auth/signup" onClick={() => setShowMobileMenu(false)} className="block bg-(--accent) hover:bg-orange-600 text-white font-bold text-center rounded-md px-3 py-2 transition-colors">GET STARTED</Link>
                            </>
                        )}
                    </div>
                </div>
