...
export const TrialStatus: React.FC = () => {
  ...
  if (isLoading || !subscription || subscription.plan !== 'founding' || subscription.status !== 'trialing') {
    return null;
  }
  ...
  return (
    <Card className={`p-4 border-l-4 ${isCritical
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Clock className={`w-5 h-5 ${isCritical ? 'text-red-600' : isUrgent ? 'text-orange-600' : 'text-blue-600'
            }`} />
          <div>
      ...
      <div className="mt-3 h-2 bg-gray-200 rounded-full overflow-hidden">
        <div
          className={`h-full transition-all ${isCritical
            ? 'bg-red-500'
            : isUrgent
              ? 'bg-(--accent)'
              : 'bg-blue-500'
      ...
      {isCritical && (
        <p className="mt-3 text-sm text-red-700 font-medium">
          Your trial expires in {daysRemaining} days. Submit a testimonial or add billing information to continue.
        </p>
      )}
      {isUrgent && !isCritical && (
