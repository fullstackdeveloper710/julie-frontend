...
export default function CreateManagerPage() {
    ...
    // Admins cannot invite further admins — only the Account Holder owns seats.
    useEffect(() => {
        if (isAdmin) router.replace('/dashboard');
    }, [isAdmin, router]);

    const { data: listResp, isLoading: isListLoading } = useListManagersQuery(
        undefined,
        { skip: isAdmin }
    );
        ...
        <div className="px-7 py-8 max-w-3xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-sm text-slate-400">
                    Invite a Deputy Chief, Lieutenant, or equivalent decision-maker. They&apos;ll
                    receive an email with a temporary password and verification link.
                </p>
                        ...
                        {admins.map((admin) => (
                            <li key={admin._id} className="py-3 flex items-start justify-between gap-3 flex-wrap">
                                <div className="min-w-0">
                                    ...
                                    {admin.title && (
                                        <p className="text-xs text-slate-500 mt-0.5">{admin.title}</p>
                                    )}
                                </div>
                                <span
            ...
            {canCreateMore ? (
                ...
                >
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mb-5">
                            The invite is sent to the email below.
                        </p>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-6 text-center text-slate-400 text-sm">
                    All {maxAllowed} admin seats are in use. Remove an existing admin to invite
                    another.
                </div>
            )}
        </div>
