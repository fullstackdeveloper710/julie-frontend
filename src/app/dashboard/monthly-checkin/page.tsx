...
function MonthlyCheckinReadOnly({
  record,
  canEdit,
  editsRemaining,
  onEditClick,
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          {!isReadOnly && !canEdit && (
            <p className="flex items-center gap-1.5 text-xs text-slate-500 mt-1.5">
              ...
              Next edit available{' '}
              <span className="text-slate-400 font-medium ml-1">{nextMonth}</span>
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          <SectionLabel>Fatigue Resistance & Peer Support</SectionLabel>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-5">
          ...
          <FieldRow
            label="Promotions / Leadership Development"
            value={ls.promotionsOrLeadershipDevelopmentCount}
          />
...
export default function MonthlyCheckInPage() {
  ...
  const isReadOnly = [USER_ROLE.MANAGER, USER_ROLE.DEPARTMENT_USER].includes(
    userResp?.data?.role as USER_ROLE,
  );
  ...
  // No existing check-in this month and user can submit
  ...
  // Read-only view with optional edit button
  if (currentRecord) {
    const editsRemaining = (status?.maxEdits ?? 2) - (status?.editCount ?? 0);
    return (
      <div className="px-7 py-8">
        <MonthlyCheckinReadOnly
